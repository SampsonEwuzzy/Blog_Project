...
def post_list(request):
    ...
    latest_posts = posts[:6]  # 6 most recent posts for carousel
...
def post_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # DEBUG: log form errors to console / Render logs
            print("POST data:", request.POST)
            print("FILES:", request.FILES)
            print("Form errors (create):", form.errors.as_json())
    else:
        form = PostForm()
...
def post_update(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            print("POST data:", request.POST)
            print("FILES:", request.FILES)
            print("Form errors (update):", form.errors.as_json())
    else:
...
def like_post(request, slug):
    ...
    if post.likes.filter(user=user).exists():
        post.likes.filter(user=user).delete()  # Unlike
    else:
